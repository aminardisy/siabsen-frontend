          ...
          <table v-else class="w-full text-left">
            ...
            <tbody class="divide-y divide-slate-100">
              ...
              <tr v-for="item in riwayatData" :key="item.id" class="hover:bg-slate-50/80 transition-colors">
                ...
                <td class="px-6 py-4 text-center">
                  <button v-if="item.statusApproval !== 'CLOSED'" @click="openEditModal(item)" class="px-4 py-1.5 bg-white border border-slate-200 hover:bg-[#26A69A] hover:text-white hover:border-[#26A69A] text-slate-600 rounded-lg text-xs font-bold transition-all shadow-sm">
                    Edit
                  </button>
                  <span v-else class="text-xs text-gray-300 font-medium">Locked</span>
                </td>
              </tr>
            </tbody>
          </table>
