...
<template>
  <div class="p-8 w-full min-h-screen bg-gray-50 font-inter">
    <div class="flex justify-between items-end mb-8">
      ...
      <button
        @click="openModal('add')"
        class="bg-[#26A69A] text-white px-6 py-2.5 rounded-xl font-bold shadow-lg shadow-teal-200 flex items-center gap-2"
      >
        ...
        <tbody class="divide-y divide-gray-50">
          ...
          >
            ...
            <td class="px-6 py-4 text-center">
              <span
                :class="
                  acc.status === 'Aktif' ? 'bg-teal-50 text-[#26A69A]' : 'bg-red-50 text-red-500'
                "
            ...
            <td class="px-6 py-4 text-center">
              <div class="flex justify-center gap-3">
                ...
                <button
                  @click="handleDelete(acc.id, acc.nama)"
      ...
      <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        <div class="bg-[#26A69A] p-6 text-white flex justify-between items-center">
          <h3 class="text-xl font-bold">
            {{ modalMode === 'add' ? 'Buat Akun Baru' : 'Edit Akun' }}
          </h3>
        ...
        <form @submit.prevent="handleSubmit" class="p-8 space-y-5">
          <div>
            ...
            <input
              v-model="formAccount.nama"
              type="text"
              class="w-full bg-gray-50 border p-3 rounded-xl outline-none focus:ring-2 focus:ring-[#26A69A]"
              required
            />
          </div>
          <div>
          ...
          <div class="flex gap-4 pt-6">
            ...
            <button type="submit" class="flex-1 py-3 bg-[#1A2342] text-white rounded-xl font-bold">
              Simpan
            </button>
          </div>
        </form>
