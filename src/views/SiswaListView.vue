...
import type { SiswaResponse, SiswaRequest } from '@/models/siswa' // Gunakan interface baru
...
// State untuk Form (Sesuai SiswaRequestDTO)
...
// 2. Logic: Search Filter (Menggunakan s.namaKelas sesuai DTO baru)
...
// Fungsi pembantu untuk mencari nama Wali Kelas dari daftarKelas (karena di DTO Siswa tidak ada)
const getWaliKelas = (namaKelas: string) => {
  const kelas = daftarKelas.value.find(k => k.namaKelas === namaKelas)
  return kelas ? kelas.namaWaliKelas : '-'
}
...
const handleSubmit = async () => {
  try {
    ...
    if (modalMode.value === 'add') {
      ...
      await siswaService.update(formSiswa.value.id!, payload)
...
const openModal = (mode: 'add' | 'edit', data: SiswaResponse | null = null) => {
  ...
  if (mode === 'edit' && data) {
    // Cari kelasId berdasarkan namaKelas dari data response
