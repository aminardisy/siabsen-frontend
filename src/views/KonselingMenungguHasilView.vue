...
import type { KonselingMenungguItem } from '@/models/konseling'

const router = useRouter()
const items = ref<KonselingMenungguItem[]>([])
...
const load = async () => {
  loading.value = true
  try {
    items.value = await konselingService.listMenungguUntukWali()
  } catch (e: unknown) {
    const msg =
      (e as { response?: { data?: { message?: string } } })?.response?.data?.message ||
      'Gagal memuat daftar konseling'
    toast.error(msg)
  } finally {
    loading.value = false
  }
...
const goCatat = (id: number) => {
  router.push({ name: 'konseling-catat-hasil', params: { id: String(id) } })
}
  ...
  <div class="min-h-screen bg-slate-50 p-6 md:p-10 font-sans">
    <div class="max-w-5xl mx-auto">
      <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        ...
        <button
          type="button"
          class="text-sm font-bold text-[#26A69A] hover:underline self-start md:self-auto"
          @click="load"
        >
          Muat ulang
        </button>
        ...
        <table v-else class="w-full text-left">
          ...
          <tbody class="divide-y divide-slate-100">
            ...
            <tr v-for="row in items" :key="row.id" class="hover:bg-slate-50/80">
              <td class="px-6 py-4">
                <p class="font-semibold text-slate-800 text-sm">{{ row.studentName || '-' }}</p>
                <p class="text-xs text-slate-400 font-mono">ID konseling: {{ row.id }}</p>
              </td>
              <td class="px-6 py-4 text-sm text-slate-600">{{ formatDate(row.date) }}</td>
              <td class="px-6 py-4 text-sm text-slate-700 max-w-xs truncate" :title="row.topic">{{ row.topic }}</td>
              <td class="px-6 py-4 text-center">
                <button
                  type="button"
                  class="bg-[#26A69A] text-white text-xs font-bold px-4 py-2 rounded-lg hover:bg-teal-600 transition shadow-sm"
